...
# input_guide.plot_mode(0, 'Int')
# input_guide.plot_mode(1, 'Int')
# check the mode profile of even/odd modes
...
Power_mode_1_2 = []
# trying to get the output power of each section
for i in range(num):
    Amplitude_0 = taper.get_element_at_position(step * i + step / 2.).get_output_right(0)
    Amplitude_1 = taper.get_element_at_position(step * i + step / 2.).get_output_right(1)
    power_mode_1_2 = (np.abs(Amplitude_0))**2 + (np.abs(Amplitude_1))**2
    Power_mode_1_2.append(power_mode_1_2)
    print 'i =', i, 'power_mode_1_2 =', power_mode_1_2

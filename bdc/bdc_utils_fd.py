...
def taper_boundaries(w1, w3, l, wg_gap, wg_width,
                     num):  # 2 additional variables need to be added: the lengths of input/output wgs
    z = np.linspace(0, l, 5)
    y1 = np.array(
        [wg_gap / 2. + wg_width, wg_gap / 2. + w1, wg_gap / 2. + w1, wg_gap / 2. + w1, wg_gap / 2. + wg_width])
    y3 = np.array(
        [-wg_gap / 2. - wg_width, -wg_gap / 2. - w3, -wg_gap / 2. - w3, -wg_gap / 2. - w3, -wg_gap / 2. - wg_width])
...
def simulate_bdc(wavelengths, wg_width=0.3e-6, wg_gap=0.2e-6, w1=0.2829e-6, w3=0.4365e-6, t_length=16.8367e-6):
    power_ratio_list = []
    trans_list = []
    # wavelength
    for wl in wavelengths:
        reme.set_wavelength(wl)
        ...
        m_si = reme.TabulatedMaterial("si_luxpop.txt")
        m_sio2 = reme.MaterialSiO2()
        num = 50

        z_new, y1, y2, y3, y4 = taper_boundaries(w1=w1, w3=w3, l=t_length, wg_gap=wg_gap, wg_width=wg_width, num=num)
        ...
        input_guide = reme.FDStraight(input_rwg, nx=501, ny=501)
        ...
        input_section = reme.WaveguideSection(input_guide, 5e-6)
        output_section = reme.WaveguideSection(input_guide, 5e-6)
        ...
        coupler.add_right_port(clad_width + wg_width * 1.5 + wg_gap, 0.2e-6 + wg_width,
                               5)  # need to be changed to 1.5*wg_width
        ...
        # plot the field
        power_l = abs(coupler.get_right_output_port(0, 0)) ** 2
        power_u = abs(coupler.get_right_output_port(1, 0)) ** 2
        print("Lower output power: {:.2f}".format(power_l))
        print("Upper output power: {:.2f}".format(power_u))
        power_ratio = power_l / (power_u + power_l)
        trans = power_u + power_l
    ...
    return power_ratio_list, trans_list


if __name__ == "__main__":
    power_ratio_list, trans_list = simulate_bdc(wavelengths=[1.31e-6], wg_width=0.38e-6, wg_gap=0.2e-6,
                                                w1=3.8e-07, w3=3.8e-07, t_length=1.5e-05
                                                )
